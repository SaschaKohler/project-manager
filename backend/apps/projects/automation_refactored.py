...
class TaskAutomationEngine(BaseAutomationEngine[Task]):
    ...
    def trigger_label_added(self, task: Task, label: TaskLabel) -> list[TaskAutomationLog]:
        """Trigger automation when a label is added to task."""
        rules = self._get_rules(task, TaskAutomationRule.TriggerType.LABEL_ADDED)
        filtered_rules = [
            rule for rule in rules
            if TriggerFilter.label_matches(rule, label)
        ]
        return self._execute_rules(filtered_rules, task)
    
    def trigger_label_removed(self, task: Task, label: TaskLabel) -> list[TaskAutomationLog]:
    ...
    def trigger_assigned_to_user(self, task: Task, user: "User") -> list[TaskAutomationLog]:
    ...
    def trigger_due_date_approaching(
        ...
        rules = self._get_rules(task, TaskAutomationRule.TriggerType.DUE_DATE_APPROACHING)
        filtered_rules = [
            rule for rule in rules
    ...
    def trigger_due_date_overdue(
        ...
        filtered_rules = [
            rule for rule in rules
            if TriggerFilter.interval_matches(rule, days_overdue)
        ]
        return self._execute_rules(filtered_rules, task)
